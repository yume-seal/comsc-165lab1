...
void serialiseCoursesDown(Course courses[], int& size)
{
    ...
    fout.open("courses.dat", std::ios::binary | std::ios::out);
    ...
    for(int i = 0; i < size + 1; i++)
    {
        if(!courses[i].courseName.empty())
        {
            ...
            fout.write(reinterpret_cast<char*>(courses), (size) * sizeof(Course));
        }
    }
    fout.close();
}
...
void serialiseCoursesUp(Course courses[], int& size)
{
    ...
    fin.open("courses.dat", std::ios::binary | std::ios::in);
